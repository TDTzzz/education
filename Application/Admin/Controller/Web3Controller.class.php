<?php
  ...
  class Web3Controller extends CommonController{
      ...
      if(IS_POST){
        if($article->create()){
          ...
          if($article->add($insert)){
            $this->success('添加文章成功',U('/Admin/Web3/index',array('type'=>I('post.type'))));
          }else{
            $this->error('添加文章失败');
          }
        }else{
    ...
    public function edit(){
      ...
      if(IS_POST){
        if($article->create()){
          ...
          if($article->save()){
            $this->success('修改文章成功',U('/Admin/Web3/index',array('type'=>$type)));
          }else{
            $this->error('修改文章失败');
          }
        }else{
