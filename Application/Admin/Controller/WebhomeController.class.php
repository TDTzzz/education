<?php
  ...
  class WebhomeController extends CommonController{
      ...
      if(IS_POST){
        if($article->create()){
          ...
          if($article->add($insert)){
            $this->success('添加文章成功',U('/Admin/Webhome/index',array('type'=>I('post.type'))));
          }else{
            $this->error('添加文章失败');
          }
        }else{
    ...
    public function edit(){
      ...
      if(IS_POST){
        if($article->create()){
          ...
          if($article->save()){
            $this->success('修改文章成功',U('/Admin/Webhome/index',array('type'=>$type)));
          }else{
            $this->error('修改文章失败');
          }
        }else{
    ...
    public function departmentadd(){
      ...
      if(IS_POST){
        if($article->create()){
          ...
          if($article->add($insert)){
            $this->success('添加文章成功',U('/Admin/Webhome/department',array('type'=>I('post.type'))));
          }else{
            $this->error('添加文章失败');
          }
        }else{
    ...
    public function departmentedit(){
      ...
      if(IS_POST){
        if($article->create()){
          ...
          if($article->save()){
            $this->success('修改文章成功',U('/Admin/Webhome/department',array('type'=>$type)));
          }else{
            $this->error('修改文章失败');
          }
        }else{
